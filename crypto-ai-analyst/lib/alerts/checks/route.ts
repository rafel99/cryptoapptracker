...
function isAuthorized(req: NextRequest): boolean {
  ...
  if (!secret) return true; // no secret configured — open endpoint (fine for manual testing only)

  // Vercel Cron Jobs automatically send this header when CRON_SECRET is set as an env var
  const authHeader = req.headers.get("authorization");
  if (authHeader === `Bearer ${secret}`) return true;

  // Fallback for manual triggers or external cron pingers (e.g. cron-job.org)
